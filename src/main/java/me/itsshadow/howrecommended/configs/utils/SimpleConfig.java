package me.itsshadow.howrecommended.configs.utils;
...
public class SimpleConfig extends YamlConfiguration {
	...
	public Object get(String path, Object def) {
		if (defaults != null) {
			Validate.isTrue(def == null, "The default value must be null since we use defaults from your file inside of the plugin!");

			if (super.get(path, null) == null) {
				final Object defaultValue = defaults.get(path);
				Objects.requireNonNull(defaultValue, "Default " + file + " lacks a key at '" + path + "' path");
	...
	private File extract(String path) {
		...
		Utils.log("&fCreated the file &7'&b" + file.getName() + "&7'&f.");
